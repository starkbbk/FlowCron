...
export const GlassInput = forwardRef(
  ({ label, error, className = '', startIcon: StartIcon, endAdornment, onFocus, onBlur, ...props }, ref) => {
    ...
    // Spacing rules: Input height = 48px, padding-left/right = 16px (adjusted for icons/adornments)
    const padLeft = StartIcon ? '48px' : '16px'
    const padRight = endAdornment ? '50px' : '16px'
    ...
    return (
      <div className={`flex flex-col gap-[8px] ${className}`}>
        {label && (
          <label className="text-[12px] font-bold text-[#fafafa] uppercase tracking-widest ml-1 opacity-90 leading-none">
            {label}
          </label>
        )}
        <div className="relative">
          {StartIcon ? (
            <StartIcon
              aria-hidden
              className={`absolute left-4 top-1/2 z-[1] -translate-y-1/2 transition-colors duration-200 ${
